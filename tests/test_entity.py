...
from custom_components.irrigation_unlimited.const import ATTR_ENABLED, ATTR_INDEX, DOMAIN, COORDINATOR
...
from tests.iu_test_support import quiet_mode, begin_test, run_until, finish_test, test_config_dir, check_summary
...
async def test_entity(hass: ha.HomeAssistant, skip_dependencies, skip_history):
    ...
    next_time = await run_until(hass, coordinator, start_time, datetime.fromisoformat("2021-01-04 06:02:00+00:00"), True)
    ...
    assert(s.attributes["next_start"] == datetime.fromisoformat("2021-01-04 06:03:00+00:00"))
    ...
    assert(s.attributes["next_start"] == datetime.fromisoformat("2021-01-04 06:05:00+00:00"))
    ...
    assert(s.attributes["next_start"] == datetime.fromisoformat("2021-01-04 06:10:00+00:00"))
    assert(hms_to_td(s.attributes["next_duration"]) == timedelta(minutes=12))
    assert(s.attributes[ATTR_FRIENDLY_NAME] == "Second zone")
    assert(s.attributes[ATTR_ICON] == "mdi:valve-closed")

    next_time = await run_until(hass, coordinator, start_time, datetime.fromisoformat("2021-01-04 06:04:00+00:00"), True)
    s = hass.states.get('binary_sensor.irrigation_unlimited_c1_m')
    ...
    assert(s.attributes["current_start"] == datetime.fromisoformat("2021-01-04 06:03:00+00:00"))
    ...
    assert(s.attributes["next_start"] == datetime.fromisoformat("2021-01-04 06:08:00+00:00"))
    ...
    next_time = await run_until(hass, coordinator, next_time, datetime.fromisoformat("2021-01-04 06:07:00+00:00"), True)
    s = hass.states.get('binary_sensor.irrigation_unlimited_c1_m')
    assert(s.state == STATE_ON)
    assert(s.attributes[ATTR_ENABLED] == True)
    assert(s.attributes[ATTR_INDEX] == 0)
    ...
    assert(s.attributes["current_start"] == datetime.fromisoformat("2021-01-04 06:05:00+00:00"))
    ...
    assert(s.attributes["next_start"] == datetime.fromisoformat("2021-01-04 14:05:00+00:00"))
    ...
    next_time = await run_until(hass, coordinator, start_time, datetime.fromisoformat("2021-01-04 06:12:00+00:00"), True)
    s = hass.states.get('binary_sensor.irrigation_unlimited_c1_m')
    assert(s.state == STATE_ON)
    assert(s.attributes[ATTR_ENABLED] == True)
    assert(s.attributes[ATTR_INDEX] == 0)
    ...
    assert(s.attributes["current_start"] == datetime.fromisoformat("2021-01-04 06:10:00+00:00"))
    ...
    assert(s.attributes["next_start"] == datetime.fromisoformat("2021-01-04 14:10:00+00:00"))
    assert(hms_to_td(s.attributes["next_duration"]) == timedelta(minutes=22))
    assert(s.attributes[ATTR_FRIENDLY_NAME] == "Second zone")
    assert(s.attributes[ATTR_ICON] == "mdi:valve-open")

    next_time = await run_until(hass, coordinator, start_time, datetime.fromisoformat("2021-01-04 06:17:00+00:00"), True)
    s = hass.states.get('binary_sensor.irrigation_unlimited_c1_m')
    assert(s.state == STATE_ON)
    assert(s.attributes[ATTR_ENABLED] == True)
    assert(s.attributes[ATTR_INDEX] == 0)
    ...
    assert(s.attributes["current_start"] == datetime.fromisoformat("2021-01-04 06:08:00+00:00"))
    ...
    assert(s.attributes["next_zone"] == 1)
    assert(s.attributes["next_name"] == "First zone")
    assert(s.attributes["next_start"] == datetime.fromisoformat("2021-01-04 14:03:00+00:00"))
    ...
    next_time = await run_until(hass, coordinator, start_time, datetime.fromisoformat("2021-01-04 06:23:00+00:00"), True)
    s = hass.states.get('binary_sensor.irrigation_unlimited_c1_m')
    assert(s.state == STATE_ON)
    assert(s.attributes[ATTR_ENABLED] == True)
    assert(s.attributes[ATTR_INDEX] == 0)
    ...
    next_time = await run_until(hass, coordinator, next_time, datetime.fromisoformat("2021-01-04 06:25:00+00:00"), True)
    s = hass.states.get('binary_sensor.irrigation_unlimited_c1_m')
    assert(s.state == STATE_OFF)
    assert(s.attributes[ATTR_ENABLED] == True)
    assert(s.attributes[ATTR_INDEX] == 0)
